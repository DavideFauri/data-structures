...
import OrderedTree (OrderedTree, insert, inorder, fromList)
...
isOrdered t = let traversal = inorder t
  in traversal == sort traversal

-- TESTS

testOrderedTree :: TestTree
testOrderedTree = testGroup "Ordered Tree" [ testOrderedGeneration
                                           , testOrderedInsert
                                           , testOrderedMerge
                                           ]

testOrderedGeneration :: TestTree
testOrderedGeneration = testProperty "generates correctly" $
...
testOrderedInsert = testProperty "preserves ordering on inserts" $
...
testOrderedMerge = testProperty "preserves ordering on merges" $
